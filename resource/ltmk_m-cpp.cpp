...
int ErrorCode;
    ...
    if ((_VTMK4Arg = ioctl(_hVTMK4VxD, TMK_IOCGetVersion, 0)) < 0 || _VTMK4Arg < TMK_VERSION_MIN) {
        close(_hVTMK4VxD);
        _hVTMK4VxD = 0;
...
ULONG _VTMK4Arg[2];
DWORD dwrd;
    dwrd = (DWORD)bcAddr | ((DWORD)cwLength << 16);
    _VTMK4Arg[0] = (ULONG)dwrd;
...
ULONG _VTMK4Arg[2];
DWORD dwrd;
    dwrd = (DWORD)rtAddr | ((DWORD)cwLength << 16);
    _VTMK4Arg[0] = (ULONG)dwrd;
    _VTMK4Arg[1] = (ULONG)pcBuffer;
#ifdef USE_TMK_ERROR
    tmkError =
...
ULONG _VTMK4Arg[2];
DWORD dwrd;
    dwrd = (DWORD)(rtDir | rtFlagMin) | ((DWORD)(rtDir | rtFlagMax) << 16);
    _VTMK4Arg[0] = (ULONG)dwrd;
    _VTMK4Arg[1] = (ULONG)pcBuffer;
#ifdef USE_TMK_ERROR
    tmkError =
